...
-- Main Compatibility Code
	...
	INSERT INTO MinorCivLeaders
		...
		SELECT	'MINOR_CIV_AL_TIRABIN',			'al_tirabin_leadericon.dds',	'the Bedouins',									'Hamad Pasha as-Sufi',					'Leader',				'Grant'								WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
		...
		SELECT	'MINOR_CIV_ANDORRA',			'andorra_leadericon.dds',		'the Principality of Andorra',					'Roger-Bernard III',					'Count',				'Grant'								WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
		...
		SELECT	'MINOR_CIV_BAGHCASARAY',		'baghcasaray_leadericon.dds',	'the Crimean Khanate',							'Haci I Giray',							'Khan',					'Darth'								WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
		SELECT	'MINOR_CIV_BALLEY_CHASHTAL',	'balley_leadericon.dds',		'the Kingdom of Mann and the Isles',			'Somerled',								'King',					'Davey_henninger'					WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
		...
		SELECT	'MINOR_CIV_KAGYRGYN',			'kagyrgyn_leadericon.dds',		'the Chukchi',									'Lawtiliwadlin',						'Principal Warrior',	'TPangolin'							WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
		...
		SELECT	'MINOR_CIV_MATONJENI',			'matonjeni_leadericon.dds',		'BaKalanga',									'Njenjema',								'Founding Priest',		'LastSword'							WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
		...
		SELECT	'MINOR_CIV_NGAZARGAMU',			'ngazargamu_leadericon.dds',	'the Kanem-Bornu Empire',						'Idris Alooma',							'Mai',					'Janboruta'							WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
		...
		SELECT	'MINOR_CIV_SANANDAJ',			'sanandaj_leadericon.dds',		'the Principality of Ardalan',					'Bani Ardalan',							'King',					'Darth'								WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
		...
		SELECT	'MINOR_CIV_SARNATH',			'sarnath_leadericon.dds',		'the Maurya Empire',							'Ashoka',								'Emperor',				'janboruta'							WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL
		...
		SELECT	'MINOR_CIV_VADUZ',				'vaduz_leadericon.dds',			'the Principality of Liechtenstein',			'Johann II',							'Prince',				'DuskJockey'						WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
...
--[ALT]	SELECT	'MINOR_CIV_ANDORRA',			'andorra_leadericon2.dds',		'the Principality of Andorra',					'Roger-Bernard III',					'Count',				'Kiang'								WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
...
--[ALT]	SELECT	'MINOR_CIV_SARNATH',			'sarnath_leadericon2.dds',		'the Maurya Empire',							'Ashoka',								'Emperor',				'LastSword'							WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL
