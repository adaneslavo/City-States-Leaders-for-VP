...
--	SELECT		'MINOR_CIV_AUCKLAND',		'wellington_leadericon2.dds',	'New Zealand',						'Henry Sewell',					'Prime Minister',		'TPangolin'			WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL
...
--	SELECT		'MINOR_CIV_GRANADA',		'granada_leadericon2.dds',		'the Emirate of Granada',			'Muhammad I ibn Yusuf',			'Emir',					'Mosile'			WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL
	...
	SELECT		'MINOR_CIV_CLERMONT',		'clermont_leadericon.dds',		'Clermont-Ferrand',					'Urban II',						'Pope',					'DarthKyofu'		WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
--	SELECT		'MINOR_CIV_CLERMONT',		'clermont_leadericon2.dds',		'Clermont-Ferrand',					'Urban II',						'Pope',					'Mosile'			WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
	...
	SELECT		'MINOR_CIV_ARMAGH',			'armagh_leadericon.dds',		'Gaelic Ireland',					'Saint Patrick',				'',						'Mosile'			WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
...
--	SELECT		'MINOR_CIV_PHANOTEUS',		'phanoteus_leadericon2.dds',	'Phocis',							'Onomarchus',					'General',				'Merrick'			WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
	SELECT		'MINOR_CIV_MANAGUA',		'managua_leadericon.dds',		'the Republic of Nicaragua',		'Augusto César Sandino',		'',						'Senshi'			WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
--	SELECT		'MINOR_CIV_MANAGUA',		'managua_leadericon2.dds',		'the Republic of Nicaragua',		'Anastasio Somoza Garcia',		'President',			'Mosile'			WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
...
--	SELECT		'MINOR_CIV_FAYA',			'faya_leadericon2.dds',			'the Republic of Chad',				'Idriss Deby',					'President',			'adan_eslavo'		WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
	...
	SELECT		'MINOR_CIV_QUELIMANE',		'quelimane_leadericon.dds',		'the Republic of Mozambique',		'Manuel de Araújo',				'Mayor',				'adan_eslavo'		WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
...
--	SELECT		'MINOR_CIV_TAIWAN',			'taipei_leadericon2.dds',		'the Republic of China',			'Sun Yat-sen',					'President',			'JakeWalrusWhale'	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
...
--	SELECT		'MINOR_CIV_SANAA',			'sanaa_leadericon.dds',			'Sheba',							'Yahya',						'Imam',					'adan_eslavo'		WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
...
--	SELECT		'MINOR_CIV_SARNATH',		'sarnath_leadericon2.dds',		'the Maurya Empire',				'Ashoka',						'Emperor',				'janboruta'			WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
...
--	SELECT		'MINOR_CIV_ANDORRA',		'andorra_leadericon.dds',		'the Principality of Andorra',		'Roger-Bernard III',			'Count',				'Kiang'				WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
--	SELECT		'MINOR_CIV_ANDORRA',		'tintagel_leadericon2.dds',		'the Principality of Andorra',		'Roger-Bernard III',			'Count',				'DuskJockey'		WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1) UNION ALL	
	...
	SELECT		'MINOR_CIV_CANOSSA',		'canossa_leadericon.dds',		'the March of Tuscany',				'Matilda',						'Margravine',			'DuskJockey'		WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='CSL-UCS' AND Value= 1);	